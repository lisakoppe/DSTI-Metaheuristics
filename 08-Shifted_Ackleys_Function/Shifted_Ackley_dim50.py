# Continuous Optimization: Shifted Ackley's Function (F6) with D=50
...
def solver(dimension, lower_bound, upper_bound, optim, bias, popsize):
    ...
    algo = pg.algorithm(pg.pso(gen=5000, omega=0.6, eta1=1.5, eta2=2.5, max_vel=0.5, variant=1))
...
def plot_fitness(solution):
    ...
    plt.plot(solution)
    plt.title("Continuous Optimization: Shifted Ackley's Function (F6) with D=50", fontsize=16)
    plt.xlabel("Time (iterations)", fontsize=12)
    plt.ylabel("Fitness", fontsize=12)
    plt.savefig("Screenshots/Ackley_convergence_curve50.png")
    plt.show()
...
sol_df.to_csv("Ackley_sol50.csv", sep=",")
print("\n    - Solutions:", sol_df)
