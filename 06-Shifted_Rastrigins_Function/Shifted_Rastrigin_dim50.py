# Continuous Optimization: Shifted Rastrigin's Function (F4) with D=50
...
def solver(dimension, lower_bound, upper_bound, optim, bias, popsize):
    ...
    algo = pg.algorithm(pg.sga(gen=2000, cr=0.9, eta_c=2.0, m=0.02, param_m=1.0, param_s=2, crossover='binomial', mutation='gaussian', selection='truncated'))
...
def plot_fitness(solution):
    ...
    plt.plot(solution)
    plt.title("Continuous Optimization: Shifted Rastrigin's Function (F4) with D=50", fontsize=16)
    plt.xlabel("Time (iterations)", fontsize=12)
    plt.ylabel("Fitness", fontsize=12)
    plt.savefig("Screenshots/Rastrigin_convergence_curve50.png")
    plt.show()
...
sol_df.to_csv("Rastrigin_sol50.csv", sep=",")
print("\n    - Solutions:", sol_df)
